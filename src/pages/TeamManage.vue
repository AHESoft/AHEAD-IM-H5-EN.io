<template>
  <div class='g-inherit m-article p-teammanager'>
    <x-header class="m-tab" :left-options="{backText: ' '}">
      <h1 class="m-tab-top">Group settings</h1>
      <a slot="left"></a>
    </x-header>
    <div class='m-body'>
      ...
      <template v-if="teamInfo && teamInfo.type==='advanced'">
        ...
        <group class='m-group'>
          <cell title="Members" :value="`共${teamMemberNum}人`" is-link :link='`/teammembers/${teamId}`'></cell>
        ...
        <group class='m-group'>
          ...
          <cell title="群Nickname" :value="nickName" @click.native="()=>onEditItemClick('修改群Nickname', 'text', 'nickInTeam', true)" is-link></cell>
          <cell title="Introduction" :value="teamInfo.intro || '未设置'" @click.native="()=>onEditItemClick(hasEditPermission?'修改Introduction':'Introduction', 'textarea', 'intro')" is-link></cell>
        </group>
