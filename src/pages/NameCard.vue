<template>
  <div class="g-inherit m-article p-namecard">
    ...
    <div v-else class="m-list">
      <group class="u-card">
        <cell :title="userInfo.account" :inline-desc="'Nickname: '+userInfo.nick" :value="userInfo.gender=='不显示'?'':userInfo.gender">
          <img class="icon" slot="icon" width="20" :src="userInfo.avatar">
        </cell>
      </group>
      <group class="u-card">
        <cell title="Gender">{{userInfo.gender}}</cell>
        <cell title="Birthday">{{userInfo.birth}}</cell>
        <cell title="Mobile">{{userInfo.tel}}</cell>
        <cell title="Mail">{{userInfo.email}}</cell>
        <cell title="Saying">{{userInfo.sign}}</cell>
      </group>
